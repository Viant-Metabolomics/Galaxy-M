...
if isempty(matches)
	sprintf('No classlists by that name, please try again! Quitting')
	return
elseif length(matches)>1
	sprintf('Multiple classlists by that name, please clean dataset object! Quitting')
	return
end
...
if asnum %boolean positive means return integers
    ...
    if isempty(classes)
        sprintf('No samples included! Quitting')
	return
    else
    ...
    if isempty(classes)
        sprintf('No classes included! Quitting')
        return
    else
